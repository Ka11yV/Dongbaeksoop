      ...
      <main class="pt-24 pb-12 min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <c:if test="${not empty successMessage}">
            <div class="mb-6 p-4 rounded-xl bg-green-50 border border-green-100 text-green-700 flex items-center gap-2">
          ...
          <c:if test="${not empty errorMessage}">
            <div class="mb-6 p-4 rounded-xl bg-red-50 border border-red-100 text-red-700 flex items-center gap-2">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div class="mb-8 p-8 rounded-2xl bg-white border border-gray-100 shadow-sm">
              ...
              <div class="space-y-6">
                <div class="flex items-center justify-between">
                  ...
                  <!-- Notice Toggle -->
                  <div class="toggle-wrapper cursor-pointer" data-target="noticeAlert">
                    <input type="checkbox" name="noticeAlert" id="noticeAlert" class="hidden"
                      ${user.isNoticeAlertEnabled(true) ? 'checked' : '' }>
                    <div
                      class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${user.isNoticeAlertEnabled(true) ? 'bg-primary' : 'bg-gray-200'}">
                      <span
                        class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${user.isNoticeAlertEnabled(true) ? 'translate-x-6' : 'translate-x-1'}"></span>
                    </div>
                  </div>
                </div>
            ...
            <div class="mb-8 p-8 rounded-2xl bg-white border border-gray-100 shadow-sm">
              ...
              <div class="space-y-6">
                <div class="flex items-center justify-between">
                  ...
                  <!-- Assignment Toggle -->
                  <div class="toggle-wrapper cursor-pointer" data-target="assignmentAlert">
                    <input type="checkbox" name="assignmentAlert" id="assignmentAlert" class="hidden"
                      ${user.isAssignmentAlertEnabled() ? 'checked' : '' }>
                    <div
                      class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${user.isAssignmentAlertEnabled() ? 'bg-primary' : 'bg-gray-200'}">
                      <span
                        class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${user.isAssignmentAlertEnabled() ? 'translate-x-6' : 'translate-x-1'}"></span>
                    </div>
                  </div>
                </div>
            ...
            <div class="mb-8 p-6 rounded-2xl bg-white border border-gray-100 shadow-sm flex items-start gap-4">
              ...
              <div>
                ...
                <p class="text-gray-600">모든 알림은 회원가입 시 등록한 학교 이메일(<span
                    class="text-primary font-bold">${sessionScope.loggedInUser.email}</span>)로 발송됩니다.</p>
              </div>
            </div>
